...
void tsgd_nuevo_subgrupo(const char* descripcion){
  printf(AMARILLO "    └───%s", descripcion);
  while(*(descripcion++)) printf(" ");
  printf(BLANCO "\n");
}

void tsgd_espaciar_subgrupo(){
  printf("\n");
}

int tsgd_mostrar_reporte(){
  if(__tsgd_cantidad_de_pruebas_fallidas == 0){
    printf(CYAN "\n------------------------------------\n"
         "%i pruebas corridas, "VERDE"%i"CYAN" errores - %s\n" NORMAL,
         __tsgd_cantidad_de_pruebas_corridas,
         __tsgd_cantidad_de_pruebas_fallidas,
         __tsgd_cantidad_de_pruebas_fallidas == 0 ? VERDE"OK"NORMAL : ROJO"D:"NORMAL);
  }
  else{
    printf(CYAN "\n------------------------------------\n"
         "%i pruebas corridas, "ROJO"%i"CYAN" %s - %s\n" NORMAL,
         __tsgd_cantidad_de_pruebas_corridas,
         __tsgd_cantidad_de_pruebas_fallidas,
         __tsgd_cantidad_de_pruebas_fallidas == 1 ? "error" : "errores",
         __tsgd_cantidad_de_pruebas_fallidas == 0 ? VERDE"OK"NORMAL : ROJO"D:"NORMAL);
  }
